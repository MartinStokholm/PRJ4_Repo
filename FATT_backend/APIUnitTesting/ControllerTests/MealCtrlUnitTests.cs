...
public class MealCtrlUnitTests
{
    ...
    public async Task Put_ReturnsStatus202()
    {
        ...
        var m = (post.Result as ObjectResult).Value as MealSimple;
        var meal2 = new MealSimpleNoId
        {
            Name = "TestMeal2",
    ...
    public async Task Put_ChangesEntryCorrectly()
    {
        ...
        var result = await _mealController.PutMeal(m.Id, meal2);
        var entry = (result as ObjectResult).Value.Adapt<MealSimpleNoId>();
        Assert.IsTrue(Tools.CompareProperties<MealSimpleNoId>(meal2, entry));
    }
